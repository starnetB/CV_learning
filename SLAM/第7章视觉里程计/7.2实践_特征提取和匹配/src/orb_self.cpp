...
/**
 * brute-force match two sets of descriptors  (暴力匹配两个描述子的集合)
 * @param desc1 the first descriptor
 * @param desc2 the second descriptor  被描述子
 * @param matches matches of two images
 */

int main(int argc, char **argv) {
    // load image  
    cv::Mat first_image = cv::imread(first_file, 0);  //加载彩色图片
    ...
    BfMatch(descriptor1, descriptor2, matches); //获取匹配结果，结果和descriptor1的长度一样
...
void ComputeORB(const cv::Mat &img, vector<cv::KeyPoint> &keypoints, vector<DescType> &descriptors) {
    ...
    const int half_boundary = 16;  //你要提取到的描述子的半径尺度
    int bad_points = 0;            //坏点数量 ，什么时坏点啊？

    //下面一部分就是提取坏描述子的方法，在图像边缘地带，小于提取的window_size/2的地方，我们认为是坏点，直接为空
    for (auto &kp: keypoints) {
        ...
        for (int i = 0; i < 8; i++) {
            ...
            for (int k = 0; k < 32; k++) {
                int idx_pq = i * 8 + k;
                //这一块是ORB定义的关键点附近的随机采点模式 [256]对[256]
            ...
            desc[i] = d;  //[256维度]BIREF信息
        }
        descriptors.push_back(desc); //放进对应的描述器里面
    }
    //报告坏点的数量
...
void BfMatch(const vector<DescType> &desc1, const vector<DescType> &desc2, vector<cv::DMatch> &matches) {
    ...
    for (size_t i1 = 0; i1 < desc1.size(); ++i1) {
        if (desc1[i1].empty()) continue; //如果，当前关键是坏点，就跳过
        cv::DMatch m{i1, 0, 256};  //当前计算点，就是i1，目标计算点从0开始，描述子最大索引256
        for (size_t i2 = 0; i2 < desc2.size(); ++i2) {
            if (desc2[i2].empty()) continue;
            int distance = 0;
            for (int k = 0; k < 8; k++) //返回当前32为字中，不同位数的个数
                distance += _mm_popcnt_u32(desc1[i1][k] ^ desc2[i2][k]); //按位异或后赋值
                //_mm_popcnt_u32 返回当前32为字中，1的个数
            }
            //得到最小的距离参数
