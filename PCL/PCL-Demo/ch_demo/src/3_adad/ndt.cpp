...
int main(int argc,char **argv){
    ...
    //即设置网格化时立方体的边长，网格大小设置在NDT中非常重要，
    //太大会导致精度不高，太小导致内存过高，并且只有两幅点云相差不大的情况才能匹配。
    //需要调，网格的体素分辨率。它需要足够大，每个体素包含与点有关的统计数据，
    //平均值，协方差等，统计数据作为一组多元高斯分布用来模拟点云，
    //并且允许我们计算和优化体素内任意位置点的存在概率。
    //该参数是与尺度最相关的，每个体素至少包含6个点，但小到足以唯一地描述环境。
    //调的经验：resolution在追求精度的情况下最好与点密度相等或者低一个数量级,这里是激光点云，所以设了1米
